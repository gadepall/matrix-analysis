...
sys.path.insert(0,'/sdcard/download/codes/CoordGeo')         #path to my scripts
#Circle Assignment
import numpy as np
...
m2=np.array([1,0]);#direction vector
q2= np.array([0,4]);
V=np.array([[1,0],[0,0]]);
u=np.array([0,-2]);
f=0;
...
#area 
def integrand1(y, a, b):
    return 2*(np.sqrt(y))
A1,err=quad(integrand1, 0, 2, args=(a,b))
#area of ellipse
#def integrand1(x, a, b):
 #   return ()
A2,err=quad(integrand1, 0, 4, args=(a,b))
area=A2-A1;
